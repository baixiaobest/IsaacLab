...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: RslRlBaseRunnerCfg):
    ...
    # The LiDAR curriculum follows the native RSL-RL iteration counter, so it
    # needs no auxiliary checkpoint state. A resumed stage must replace the
    # dense initial scan and clear the observation history before learning.
    density_collector = getattr(env.unwrapped, "_held_scan_lidar_collector", None)
    if density_collector is not None and density_collector.cfg.density_curriculum_enabled:
        stage_changed = density_collector.configure_density_schedule(
            runner.current_learning_iteration, agent_cfg.num_steps_per_env
        )
        if stage_changed:
            density_collector.reset()
