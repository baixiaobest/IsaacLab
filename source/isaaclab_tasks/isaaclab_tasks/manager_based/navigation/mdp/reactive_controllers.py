...
"""Non-learning reactive controllers for Go2 obstacle-avoidance navigation.

These were originally standalone helpers imported by ``evaluate_baseline.py`` /
``evaluate_dwa_baseline.py`` from the ``scripts/`` directory. They live here (in the installed
``isaaclab_tasks`` package) so that :mod:`reactive_pre_trained_policy_action` can also import them
as an in-environment action term, without depending on a scripts directory being on ``sys.path``.
``scripts/reinforcement_learning/rsl_rl/greedy_baseline.py`` and ``dwa_baseline.py`` re-export these
classes unchanged for backward compatibility.
"""
