...
from .test_terrain_generator import TestTerrainGenerator  # Add this import
...
class TerrainImporter:
    ...
    def __init__(self, cfg: TerrainImporterCfg):
        ...
        self._single_terrain_generator = None
        self._test_terrain_generator = None  # Add this line
        ...
        if self.cfg.terrain_type == "generator":
            ...
            self._terrain_generator = TerrainGenerator(cfg=self.cfg.terrain_generator, device=self.device)
            self.import_mesh("terrain", self._terrain_generator.terrain_mesh)
            # configure the terrain origins based on the terrain generator
            self.configure_env_origins(self._terrain_generator.terrain_origins)
            # refer to the flat patches
            self._terrain_flat_patches = self._terrain_generator.flat_patches
        ...
        elif self.cfg.terrain_type == "test_generator":  # Add this block
            # Check config is provided
            if self.cfg.test_terrain_generator is None:
                raise ValueError("Input terrain type is 'test_generator' but no value provided for 'test_terrain_generator'.")
            # Generate the terrain
            self._test_terrain_generator = TestTerrainGenerator(cfg=self.cfg.test_terrain_generator, device=self.device)
            self.import_mesh("terrain", self._test_terrain_generator.terrain_mesh)
            # Configure the terrain origins based on the test terrain generator
            self.configure_env_origins(self._test_terrain_generator.terrain_origins)
        else:
    ...
    def terrain_generator(self) -> TerrainGenerator:
        """The terrain generator used to create the terrain.
    ...
    @property
    def test_terrain_generator(self) -> TestTerrainGenerator:
        """The test terrain generator used to create the terrain.
        
        This is only available if the terrain type is 'test_generator'.
        
        Returns:
            The test terrain generator instance.
        """
        return self._test_terrain_generator
    
    def get_column_terrain_names(self) -> list[str]:
        """Get the names of the terrains in the column format.
        Only useful if the terrain is generated by the terrain generator and using curriculum.
        Returns:
            A list of terrain names in columns.
        """
        if self._terrain_generator is None:
            return []
        return self.terrain_generator.get_terrain_names()
    
    def get_env_terrain_names(self, env_ids: torch.Tensor | None = None) -> list[str]:
        """Get the names of the terrains for the given environment ids.
            This operation is slow, please call it only once and cache it.
        Args:
            env_ids: The environment ids to get the terrain names for. If None, all terrain names are returned.

        Returns:
            A list of terrain names for the given environment ids.
        """
        if env_ids is None:
            env_ids = torch.arange(self.cfg.num_envs, device=self.device)
        # get the terrain types
        terrain_types = self.terrain_types[env_ids].cpu().numpy()
        # return the terrain names
        return [self.get_column_terrain_names()[t] for t in terrain_types]
